...
static int dw9719_probe(struct i2c_client *client)
{
	...
	/*
	 * The IPU bridge instantiates the VCM asynchronously from the sensor's
	 * notifier callback. The host may have completed its initial subdev-node
	 * registration before this lens binds, so register nodes again now that
	 * the async notifier has attached us to the camera's v4l2_device.
	 */
	if (dw9719->sd.v4l2_dev) {
		node_ret = v4l2_device_register_subdev_nodes(dw9719->sd.v4l2_dev);
		if (node_ret)
			dev_err(&client->dev,
				"Failed to register VCM subdev node: %d\n", node_ret);
	}
...
/*
 * IPU bridge creates the Surface Pro 7 lens as an I2C client from ACPI and
 * names it "dw9719". Keep an I2C ID table as well as the OF table so that
 * ACPI-instantiated VCMs bind and receive the model data used by probe().
 */
static const struct i2c_device_id dw9719_i2c_ids[] = {
...
MODULE_DEVICE_TABLE(i2c, dw9719_i2c_ids);
