...
/* Defaults retain the qualified RGB configuration; override only for a
 * bounded Surface Pro 7 source-6/one-lane receiver experiment. */
static unsigned int csi2_legacy_port_config = 0x3895;
module_param(csi2_legacy_port_config, uint, 0644);
MODULE_PARM_DESC(csi2_legacy_port_config,
		 "CSI2 legacy GPREG port configuration (experimental)");

static unsigned int csi2_combo_port_config = 0x3895;
module_param(csi2_combo_port_config, uint, 0644);
MODULE_PARM_DESC(csi2_combo_port_config,
		 "CSI2 combo GPREG port configuration (experimental)");
...
static void ipu4p_isys_port_cfg(struct ipu_isys *isys)
{
	...
	void __iomem *isp_base = isys->adev->isp->base;
	/* Port config */
	writel(csi2_legacy_port_config, base + IPU_GPOFFSET +
	       CSI2_REG_CSI_GPREG_CR_PORT_CONFIG);
	writel(csi2_combo_port_config, base + IPU_COMBO_GPOFFSET +
	       CSI2_REG_CSI_GPREG_CR_PORT_CONFIG);
