...
operations, and uses endpoint open/close notifications to trigger `/proc`
consumer reconciliation. If those notifications are unavailable or their
watch is lost, it falls back to the existing 200 ms active and 1000 ms stable
idle scan intervals. Idle `videotestsrc` fillers link directly to their
